...
if [[ "$unamestr" == 'Linux' ]]; then
	...
	avr-gcc -Os -std=c11 -ffunction-sections -fdata-sections -mmcu=atmega328p -DF_CPU=16000000UL -Wl,--gc-sections Blink_main.c -o Blink -I${mdd_path[0]} -I${path_to_om} -I${path_to_mdd} -I${om_path[0]}  
	if [ $? -ne 0 ]; then
		exit 1
	fi
	...
	avrdude -F -V -c arduino -p ATMEGA328P -P $port -b $b_rate -U flash:w:Blink.hex
	if [ $? -ne 0 ]; then
		exit 1
	fi
else
