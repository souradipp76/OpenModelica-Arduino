      ...
      package led
        ...
        algorithm
          ...
          if ok <> 0 then
            ...
            sComm.delay(1000);
            digital_out := sComm.cmd_digital_out(1, 9, 0) "This will turn ON the blue LED";
            sComm.delay(5000);
          end if;
...
// At port 0 with baudrate of 115200
          if ok <> 0 then
            ...
            for i in 1:500 loop
              analog_in := sComm.cmd_analog_in(1, 5);
              if analog_in < 300 then
                digital_out := sComm.cmd_digital_out(1, 9, 1);
              else
                digital_out := sComm.cmd_digital_out(1, 9, 0);
              end if;
            end for;
          end if;
//Run for 500 iterations
// read analog pin 5 (ldr)
...
// To close the connection safely
        ...
        model ldr_read
          ...
          import Exn = Modelica.Exceptions;
          import strm = Modelica.Utilities.Streams;
          import op = ModelicaReference.Operators;
          Integer ok(fixed = false);
          Integer analog_in(fixed = false);
          Integer c_ok(fixed = false);
        algorithm
...
// read analog pin 5 (ldr)
//strm.print(op.String(analog_in));
          c_ok := sComm.close_serial(1);
// To close the connection safely
        end ldr_read;
      end ldr;
          ...
          if ok <> 0 then
            ...
            for i in 1:10 loop
              ...
              elseif analog_in >= 320 and analog_in <= 900 then
                digital_out := sComm.cmd_digital_out(1, 10, 1);
                sComm.delay(1000);
                digital_out := sComm.cmd_digital_out(1, 10, 0);
...
// At port 0 with baudrate of 115200
          if ok <> 0 then
            ...
            for i in 1:500 loop
              analog_in := sComm.cmd_analog_in(1, 4);
              if analog_in > 500 then
                digital_out := sComm.cmd_digital_out(1, 3, 1);
              else
                digital_out := sComm.cmd_digital_out(1, 3, 0);
              end if;
              sComm.delay(500);
            end for;
          end if;
...
// To close the connection safely
      ...
      package servo
        ...
        model servo_init
          ...
          Integer c_ok(fixed = false);
        algorithm
          ok := sComm.open_serial(1, 1, 115200);
//COM port is 0 and baud rate is 115200
          if ok <> 0 then
            strm.print("Check the serial port and try again");
          else
...
//COM port is 0 and baud rate is 115200
          if ok <> 0 then
            ...
            for i in 1:5000 loop
              analog_in := sComm.cmd_analog_in(1, 2);
              analog_in := sComm.mfloor(analog_in * (180 / 1023));
    ...
    package MDD_Examples "Interfacing arduino with OpenModelica using Modelica Device Drivers"
      ...
      package MDD_ldr
        ...
        model MDD_ldr_read
          ...
          Modelica.Blocks.Interaction.Show.RealValue realValue1(use_numberPort = true) annotation(
            Placement(visible = true, transformation(origin = {28, -17}, extent = {{-18, -17}, {18, 17}}, rotation = 0)));
        equation
      ...
      package MDD_dcmotor
        ...
        model MDD_dcmotor_both
          ...
          Modelica_DeviceDrivers.EmbeddedTargets.AVR.Blocks.PWM pwm(fastPWM = true, initialValues = {-1}, prescaler = Modelica_DeviceDrivers.EmbeddedTargets.AVR.Types.TimerPrescaler.'1/1024', timer = Modelica_DeviceDrivers.EmbeddedTargets.AVR.Types.TimerSelect.Timer1, timerNumbers = {Modelica_DeviceDrivers.EmbeddedTargets.AVR.Types.TimerNumber.A}) annotation(
